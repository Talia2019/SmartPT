...
package com.example.miscale_01.bluetooth;
...
//    }
    ...
    protected void writeBytes(UUID service, UUID characteristic, byte[] bytes, boolean noResponse) {
        Timber.d("Invoke write bytes [" + byteInHex(bytes) + "] on " + com.example.miscale_01.bluetooth.BluetoothGattUuid.prettyPrint(characteristic));
    ...
    void readBytes(UUID service, UUID characteristic) {
        Timber.d("Invoke read bytes on " + com.example.miscale_01.bluetooth.BluetoothGattUuid.prettyPrint(characteristic));
    ...
    protected void setIndicationOn(UUID service, UUID characteristic) {
        Timber.d("Invoke set indication on " + com.example.miscale_01.bluetooth.BluetoothGattUuid.prettyPrint(characteristic));
    ...
    protected void setNotificationOn(UUID service, UUID characteristic) {
        Timber.d("Invoke set notification on " + com.example.miscale_01.bluetooth.BluetoothGattUuid.prettyPrint(characteristic));
        if(btPeripheral.getService(service) != null) {
            stopMachineState();
            BluetoothGattCharacteristic currentTimeCharacteristic = btPeripheral.getCharacteristic(service, characteristic);
            btPeripheral.setNotify(currentTimeCharacteristic, true);
